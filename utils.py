...
def save_to_file(link, depth, concat=True, cookie=None):
    if not cookie:
    ...
    crawler(website, depth, outpath, cookie, concat=concat)
...
def is_contain_keyword(link, keyword, session, cookie):
    ...
    wlcard = 'co.ain'
    #wlcard = 'hack.'
    result = soup.find_all(text=re.compile(wlcard, re.IGNORECASE))
    return len(result) > 0  # untuk ngecek ketika ketemu keyword

'''
#metode Regex Optional
def is_contain_keyword(link, keyword, session, cookie):
    headers = random_header()
    response = session.get(link, headers=headers, cookies=cookie)
    soup = BeautifulSoup(response.content, 'html.parser')
    #optional = 'cocain?'
    optional = 'hacks?'
    result = soup.find_all(text=re.compile(optional, re.IGNORECASE))
    return len(result) > 0  # untuk ngecek ketika ketemu keyword
'''